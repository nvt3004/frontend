...
public interface OrderJPA extends JpaRepository<Order, Integer> {
	...
	@Query("SELECT o FROM Order o " + "WHERE (:name IS NULL OR :name = '' OR o.fullname LIKE %:name%) AND "
			+ "(:address IS NULL OR :address = '' OR o.address LIKE %:address%) AND "
			+ "(:status IS NULL OR :status = '' OR o.orderStatus.statusName =:status) "
			+ "ORDER BY o.orderStatus.sortOrder")
	Page<Order> findOrdersByCriteria(@Param("name") String name, @Param("address") String address,
